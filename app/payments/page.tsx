            ...
            {rows.map(({ site, contractTotal, paid, unpaid, status }) => (
              <div key={site.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
                <div className="flex items-center justify-between mb-2">
                  <Link href={`/sites/${site.id}`} className="font-semibold text-slate-800 hover:text-blue-600 truncate mr-2">
                    {site.name}
                  </Link>
                ...
                <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-100">
                  <div className="text-center">
                    <p className="text-xs text-slate-400 mb-1">계약금액</p>
                    <p className="text-sm font-medium text-slate-700">
                      {contractTotal > 0 ? contractTotal.toLocaleString() : '—'}
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-slate-400 mb-1">수금액</p>
                    <p className="text-sm font-medium text-slate-700">
                      {paid > 0 ? paid.toLocaleString() : '—'}
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-slate-400 mb-1">미수금</p>
                    <p className={`text-sm font-medium ${unpaid > 0 ? 'text-red-600' : 'text-slate-500'}`}>
